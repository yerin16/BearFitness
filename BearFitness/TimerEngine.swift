...
final class TimerEngine {
    ...
    var currentRound: Int { currentSection?.roundNumber ?? 1 }
    var totalRounds: Int { sections.map(\.roundNumber).max() ?? 1 }
    var currentInterval: Int { currentSection?.intervalNumber ?? 1 }
    ...
    private func buildSections(from program: HIITProgram) -> [TimerSection] {
        ...
        if program.warmUpSeconds > 0 {
            result.append(TimerSection(phase: .warmUp, durationSeconds: program.warmUpSeconds, roundNumber: 0, intervalNumber: 0))
        }
        ...
        for cycle in 1...cycleCount {
            for set in 1...program.intervalSets {
                if program.highIntensitySeconds > 0 {
                    result.append(TimerSection(phase: .highIntensity, durationSeconds: program.highIntensitySeconds, roundNumber: cycle, intervalNumber: set))
                }
                if program.lowIntensitySeconds > 0 {
                    result.append(TimerSection(phase: .lowIntensity, durationSeconds: program.lowIntensitySeconds, roundNumber: cycle, intervalNumber: set))
                }
            }
        }

        if program.coolDownSeconds > 0 {
            result.append(TimerSection(phase: .coolDown, durationSeconds: program.coolDownSeconds, roundNumber: 0, intervalNumber: 0))
        }
    ...
    func start() {
        ...
        // Complete immediately if all durations were 0
    ...
    private func recordCurrentSection() {
        ...
        completedSections.append(SessionSection(
            ...
            actualDurationSeconds: section.durationSeconds - secondsRemaining,
